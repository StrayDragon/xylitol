...
| 同步库；产品端 host 驱动（tokio 仅 autocomplete 异步辅助，runtime 由消费方提供） | 拥有产品事件循环 |
...
| 角色 | 路径 | 说明 |
|---|---|---|
| **产品视觉 / 设计稿** | app `DESIGN.md` 色板 + 顶层 `designing/` 交互稿 | 固定态给人看；运行时以产品代码为准 |
...
| 是 | 不是 |
|---|---|
| 包引擎 / 通用组件的动态试跑（`agent_demo` / `agent_demo_alt` / `just demo-tui*`） | 产品 TUI 或 designing 交互稿 |
| `XYLITOL_TUI_MOUSE` 仅 **Inline** demo（及以其为目标的 PTY e2e）生效；Inline 与 ApplicationOwned 用双 example 区分 | 产品 inline 鼠标开关；env 热切模式 |
| demo 屏英文 / plate 文案以 demo 自身为准（人类明确要求除外），允许与产品中文 SSOT（队列条 / 滚动提示 / 命令面板…）不同 | 为「对齐产品固定区词汇表」改写 demo 文案 |
| 形态学对照产品（rail 默认、中间队列条）；对照 ≠ 同一 SSOT | 把对照当落地 |
| 产品固定区用词：[`docs/architecture/TUI信息呈现与固定区词汇.md`](../../docs/architecture/TUI信息呈现与固定区词汇.md)——**只约束产品端文档与 host** | 用它约束本包 demo 字符串 |
| 改产品视觉 / 固定区：走 `src/app/tui/` 产品代码 + `just open-designing` 对照 / 产品测 | 默认先改 `agent_demo*` 当「落地」 |
...
缺能力时：**先在本包补通用能力，再由应用端接线**——不要把准通用实现塞进 `src/app/tui/`。
...
4. 主题用闭包注入；语义 token 映射在应用端（`src/app/tui/DESIGN.md`）。
...
| 验什么 | 在哪跑 | 命令 |
|---|---|---|
| 包组件测试（按键→状态 / snapshot / 时序；proptest 未落地，落地时补 `tests/property_test.rs`），口径同 skill `test-tui-harness` | 本包 `tests/` | `just test-tui` |
| 真终端 E2E（crossterm / PTY / tmux） | 工作区 `tests/tui_e2e/`：主场景 spawn **`agent_demo`**；另含产品 Fake smoke（`pty_product_*`，隔离 config） | `just test-tui-e2e`（或 `-pty` / `-tmux`） |
| 仓库全量门禁（不含真终端 E2E） | 全仓 | `just qa` |
| 全量门禁 + 真终端 E2E | 全仓 | `just qa-e2e` |
| 产品 host / `XyEvent` / slash 接线 | `src/app/tui/tests.rs` 等 | 随产品测；**不**替代上表 |

**分工（勿混）**

- **包 E2E / `agent_demo*`**：引擎 + 通用组件 + 真终端协议；文案以 demo 自身为准。
- **产品 TUI**：Driver / bridge / layout / 键位 → 应用端 harness。真终端 E2E 另有隔离 config 的产品 Fake smoke，日常不绑真 API。
- 真终端 E2E 全 `#[ignore]`；缺 tmux 用 `just test-tui-e2e-pty`。真终端就绪针 `DEMO_READY_NEEDLE`（`tests/tui_e2e.rs` 常量，改 demo 首帧文案须同步）。操作细则：`test-tui-harness` skill。

## Specs

本包能力 specs 使用 `package-tui-*` 前缀（见根 `AGENTS.md` 与 `llmanspec/AGENTS.md`）。产品端用 `app-tui-*`（不再堆进单体 `app-tui`）。
...
| 改产品 TUI 端 / UX / 视觉 | `l8ng-write-tui` + 产品代码 + `src/app/tui/DESIGN.md` + `just open-designing`；**勿**默认改 `agent_demo` 当产品落地 |
