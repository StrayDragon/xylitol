...
本文件只放本 package **稳定边界**。仓库规则见根 `AGENTS.md`；应用面见 `src/app/tui/AGENTS.md`。
...
对齐源：`../pi/packages/pi-tui`、`../kimi-code/packages/pi-tui`。进度与裁剪笔记见根 `_HANDOFF.md`（非规范）。
...
| 日常验证 | `cargo test -p xylitol-tui`；仓库级 `just qa`；E2E `just test-tui-e2e` |

裁剪与待补 API 随接线演进，细节以代码与 `_HANDOFF.md` 为准，不在本文件维护清单。
