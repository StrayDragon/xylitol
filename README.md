...
- **Provider attribution**: The concept of injecting attribution headers for providers like OpenRouter, NVIDIA, Cloudflare, and Vercel (frozen change `c82-provider-attribution`; retrieve via `llman sdd archive freeze --list` or [`docs/architecture/变更索引.md`](docs/architecture/变更索引.md)) was adapted from pi's `provider-attribution.ts`.
