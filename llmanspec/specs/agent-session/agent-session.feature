...
功能: agent-session
  ...
  @executable @req:r1074
  场景: auto-persist
    假如 一个 turn 完成
    当 加载会话文件
    那么 该 turn 的消息记录已保存

  @executable @req:r1061
  ...
  @executable @req:r1069
  场景: abort
    假如 agent 流式响应中
    当 调用 abort()
    那么 agent 循环终止并返回 abort 错误

  @executable @req:r1071
  ...
  @executable @req:r1075
  场景: resume-validates-cwd
    假如 会话文件 cwd 指向存在目录
    当 调用 resume_session
    那么 会话加载成功

  @executable @req:r1076
