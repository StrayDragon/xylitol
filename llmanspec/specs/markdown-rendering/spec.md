...
requirements[8]{req_id,title,statement}:
  ...
  r7,"streaming-render","System MUST use debounced incremental rendering for Markdown during LLM token streaming to avoid per-frame full reparse."
  r8,"streaming-dual-phase","System MUST support dual-phase rendering where completed paragraphs receive full Markdown precision while in-progress tail receives lightweight styling."
