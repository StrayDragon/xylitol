...
功能: app-tui-transcript
  ...
  场景: todo-tool-block-body-checklist
    - todo_* 工具块展开 body MUST 渲染清单形态：每条目一行（状态字形 + content），状态字形与待办栏共用同一张表；MUST NOT 把调用 args 或结果的原始 items JSON 当作块 body。is_error 结果 MUST 保持错误文本原样可见，MUST NOT 套用清单渲染。todo_* 成功结果解析为空列表时，展开 body MUST 为一行显式空态提示（固定英文词 `(empty list)`），MUST NOT 留空 body 静默。历史重建与直播 MUST 共用同一渲染 helper，重建形态 MUST 与直播幂等（att50 幂等语义延伸到块 body）。

  @req:r1367 @executable
  ...
  @req:r1343 @executable
  场景: todo-summary-empty-not-placeholder
    当 折叠态读取无 items 的 todo_rewrite 与带补丁 items 的 todo_update 的人话摘要
    那么 todo_rewrite 摘要 MUST 为空串（header 只画工具名）
    并且 todo_update 摘要 MUST 匹配补丁计数（如 `1 updated`），MUST NOT 为空串或三点占位
