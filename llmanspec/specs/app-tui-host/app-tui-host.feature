...
功能: app-tui-host
  ...
  场景: interactive-loop-pumps-inline
    - 交互 bang 与 reload 循环 SHALL 在既有 tick 节奏内经窄入口（drain_inline_pending）泵执行类为 Inline 的 pending effects，MUST NOT 阻塞 bang 输出 chunk 泵的既有节奏；执行类为 Exclusive 与 Queued 的 pending MUST 保持排队至循环归还后由主循环 drain_pending 处理且语义不变；主循环 drain_pending 的全量泵行为 MUST NOT 改变。

  @req:ath45 @executable
  场景: exclusive-stays-queued
    假如 交互 bang 正在运行
    当 提交执行类为 Queued 的 /session-fork
