...
功能: app-tui-commands
  ...
  场景: slash-session-lifecycle
    - 产品 TUI idle 时 MUST 解析 /session-new、/session-clone 与 /session-name：/session-new 无参 MUST 经 Driver::new_session（或等价）创建空会话并切换、清空 transcript，带参 MUST usage 错误；/session-clone 无参 MUST 对当前 leaf 以 ForkPosition::At 执行 fork_session 并 switch（对齐 pi /clone；MUST NOT 套用 /session-fork 的 Before/At 规则），无 leaf MUST 提示且 MUST NOT fork，带参 MUST usage 错误；/session-name 无参 MUST 显示当前会话名或 usage，有参 MUST 经 Driver::set_session_name（换行→空格 trim）写入并回报最终名，规范化与输入不同时 MUST 提示；busy 时 /session-new 与 /session-clone MUST 拒绝且 MUST NOT 入 steer；/session-name busy MUST 仍可执行（Allow）且 MUST NOT 入 steer；SlashCommandSource MUST 列出三者；旧名 /new /clone /name MUST NOT 被识别；MUST NOT 发明平行词表或 pi 式 user 选择器。
