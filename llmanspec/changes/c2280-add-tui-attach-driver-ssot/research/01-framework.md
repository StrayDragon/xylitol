...
> 本篇只给能力与取舍事实；「选哪个 / 否决哪些」是提案裁决，见 `proposal.md`「研究结论」。
> 负载前提：连接少（约 1–3 条）、单连接 token 级小 JSON 很密、serde 往返多、要双向流（含 server→client 反向调用）、本机 UDS、未来浏览器要能连。不是 C10k，不是 CRUD REST。
