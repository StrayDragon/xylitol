# 04 运行拓扑与 Docker 连法（能力对照，非裁决）

> 本篇摆「怎么跑、怎么连」的能力事实；默认走哪条、哪些不做（如 vsock）是提案裁决，见 `proposal.md`「研究结论」与「非目标」。

## 两种运行形态

- **本机 serve**：TUI 进程内跑操作器（今天的行为）；显式 `xylitol serve` 后 attach，本机 Linux 可以听 UDS。
- **server 整进程进 Docker = 粗粒度沙盒**：TUI 仍在宿主机，经端口连；工具 / bang / MCP 都在容器里。
...
两种形态都是「N 个 TUI ↔ 1 个 server」：多窗 = 同一 WebSocket / HTTP 上的多个订阅者，journal 已按 session 缓冲 Event。

## server 在 Docker 里时的三种连法（能力对照）

### 1. published port（发布端口）

容器内 axum 听 `0.0.0.0:PORT`，宿主机 `docker run -p 127.0.0.1:<host-port>:<container-port>`；TUI attach `http://127.0.0.1:<host-port>`（HTTP 上的 tagged JSON + WebSocket）。
...
## 与 axum listener 的对应关系（事实）

同一 Router，只换 listener：
...
## 若扩展到「1 管理面 → N server」的设计面（后续议题）

多 server 才出现的维度：工作区写锁、按 cwd 的 MCP 池、session 落在哪台、TUI 连哪把锁。现有单实例 `xylitol serve` 的锁语义是「一台 host 一把」。这些维度本身中立；是否排期是提案「非目标」的裁决。
...
- 锁 / 进程模型（提案层决策，本篇不深究）：P3 显式 serve；`/tmp/xylitol-server.lock` → 用户级 `$XDG_RUNTIME_DIR/xylitol/`，POSIX `O_EXCL` 已有
