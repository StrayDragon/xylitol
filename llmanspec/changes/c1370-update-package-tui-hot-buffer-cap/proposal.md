...
> **一句话**：差异渲染引擎 previous_lines 热缓冲封顶（默认约 rows*40），长会话内存与每帧 diff 成本有界
> **当前排序**：#10（2026-08-10 自 delayed-changes 升格入 active）


> **已升格（2026-08-10）**：当前排序 **#10**。**与 c1760**：activity-fold 减行后可能 **缓解** 引擎 `previous_lines` 压力；**不是**被 c1760 吸收实现。超长跑若仍胀，再单独评估。
