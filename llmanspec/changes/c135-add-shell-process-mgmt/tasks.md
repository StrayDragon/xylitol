...
- [ ] ~~在 `agent/bash_executor.rs` 中集成 `build_shell_env()`~~（defer: 当前 bash_executor 不需要显式 env 注入，用默认环境即可）
- [ ] ~~在 `agent/bash_executor.rs` 中集成 `wait_for_child()`~~（defer: 现有 channel-based 等待逻辑已足够；wait_for_child 作为独立 API 提供供未来使用）
...
- [ ] ~~`cargo test --test bdd`~~（pre-existing compilation errors, unrelated to this change）
