...
- [ ] 1.1 建立 `src/protocol/{wire,ports}/`；迁入 command/event/transport → wire
- [ ] 1.2 将 `src/runtime_protocol/*` 迁入 `protocol/ports/`；全仓改 import；删 `runtime_protocol` 顶栏
- [ ] 1.3 更新 `src/AGENTS.md` / 根 AGENTS：方案 B 纪律 + 依赖图
...
- [ ] 3.1 迁 `message` / Env → `protocol/message.rs`；`project_for_llm` → `agent/`；agent `pub use`
- [ ] 3.2 迁 `session_types` → `protocol/session.rs`；`lifecycle`（XyEvent）→ `protocol/lifecycle.rs`
