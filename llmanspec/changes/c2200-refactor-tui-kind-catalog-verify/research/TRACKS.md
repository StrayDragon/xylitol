...
`c2200-fold-scene` 完成后：先对齐当前 main（已含 atom），再 rebase；冲突会集中在 `activity_fold/mod.rs` / `summary.rs` / `tests.rs`。语义 dump 应走 `activity_atom` / `count_cluster`，不要再 `_ => {}`。
