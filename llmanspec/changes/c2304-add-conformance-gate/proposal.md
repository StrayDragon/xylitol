...
- 一侧 `InProcessClient`，一侧 `HttpWsClient` + c2302 真序列化与传输（POST + WS 下行），不 mock 掉协议。c2290 的 `InProcessClient` 现为 EchoHost 座位，**不是**真 dispatch；本票 MUST 换成与 Host 同表的 in-process 实现，禁止 echo 过闸。
- 面本地（`Quit`、键、画）不进表。
- 只读 / 写者冲突语义入表。
