...
## 3. 组件化：5 个 widget（src/app/tui/components/）
...
- [x] `components/mutable_line.rs`：`MutableLine` widget，pending_tail + width + area → tail buffer 顶区（wrap 多行，透明 bg `Color::Reset`）
- [x] `components/thinking_indicator.rs`：`ThinkingIndicator` widget，spinner_idx + status → 一行（input_bg）
- [x] `components/input_prompt.rs`：`InputPrompt` widget，input buffer + area → 底行（`❯` + 内容 + input_bg block + 光标，CJK 显示宽度）
- [x] `components/tail.rs`：`Tail` widget，组合上述 bottom-anchored：`[MutableLine wrapped][Thinking?][Input]`，capacity = area.height，超容量顶部丢弃；`draw_tail_frame` 退化为 `Tail::render`

## 4. harness 覆盖（每个 widget 独立 TestBackend 可测）

- [x] `TranscriptLine`：ASCII / CJK / 长 wrap 多行
- [x] `MutableLine`：wrap 多行 + 透明 bg 断言
- [x] `ThinkingIndicator`：spinner + label 行
- [x] `InputPrompt`：`❯` + 内容 + input_bg + 光标位置（含 CJK）
- [x] `Tail` 组合：流式 [Mutable][Thinking][Input] / 非流式 [Input] / 超容量顶部丢弃 / TurnEnd 无残留
