...
> **一句话**：单调 context epoch 计数，凡改前缀/工具集的操作都 bump，保证模型看到稳定世界
> **当前排序**：#14（2026-08-10 自 delayed-changes 升格入 active）


> **已升格（2026-08-10）**：当前排序 **#14**。Codex 对齐后确认——`tool_search` 搜最新 registry **不需要** epoch；动态工具经 `tool_search_output` 进轨迹，而非顶栏 `tools[]` 换代计数。epoch 仅当后续断链/可观测换代需要时执行。
