...
| 在范围 | 不在范围 |
|---|---|
| live `ath30` → 产品 MUST **默认** ApplicationOwned | 库双入口结构（属 [`c2070`](../c2070-add-package-tui-dual-interaction-modes/)） |
...
```text
c2070 archive（库入口 + 启动绑定 + 禁热切）
  → c2071 start → ath30 默认 AO → host 默认 → 文档/人验 → archive
```
