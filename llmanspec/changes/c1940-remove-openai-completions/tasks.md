...
- [ ] 1.1 `infra-provider`：改写 `pa4`/`pa6`/`pa22`；feature 去掉 Completions 双实现措辞；补「废弃 api 静默→Responses」文档场景（`feature: false`）
- [ ] 1.2 `package-ai-bridge`：`pab5`/`pab11`–`pab13`/`pab15`/`pab21` 去掉 Completions；feature 删 `openai-completions-effort`，改写 `openai-via-sdk`
- [ ] 1.3 `runtime-model-registry`：改写 `m13`/`m14` 与 Completions 场景为「残留 openai-completions → 装配 Responses」
- [ ] 1.4 `agent-hooks`：`h12` 仅 Responses + Anthropic；删 Completions 场景
- [ ] 1.5 `package-ai-bridge-accounting`：去掉 Completions-only 措辞/场景（或改为非 Responses）
- [ ] 1.6 `llman sdd validate c1940-remove-openai-completions --strict --no-check` 及触及 caps
