...
| 3–5 实现 / 探针 | ✅ | Apply backlog |
| 6 validate / verify | ✅ apply；verify 另步 | apply 门禁绿；建议 `llman-sdd-verify` |
...
- [x] 3.0 落点区域 `audit-dead-code`（write-surface 步骤 1）— 无死骨架；新模块干净入口
- [x] 3.1 `SurfaceMode::AppendOnly` + CLI `append-only` 显式进入（不抢 TTY 默认 AO）
- [x] 3.2 瘦 host：`XyDriver::run` → bridge → append-only layout；库 Inline 生命周期
- [x] 3.3 键位闭集接线；禁止注册主线 fold/slash/plate

### 4. Spill + 矮帽

- [x] 4.1 session 旁唯一 spill 目录；`full_output_path` 指向其内文件
- [x] 4.2 UI 行帽 3/5；溢出写 spill + `[Full output: …]`；无展开交互
- [x] 4.3 与工具硬截断路径对齐（`OutputAccumulator` + `app/core/tool_spill`）

### 5. 架构探针 + 验证

- [x] 5.1 ScriptedDriver harness：流式/工具/spill/abort/exit
- [x] 5.2 审查复用/不复用表（import 边界 arch_tests）
- [x] 5.3 `llman sdd validate --strict`；相关测绿 → verify
...
0 Designed ✅
  ...
  → 3 面入口 + Inline host ✅
  → 4 spill + 矮帽 ✅
  → 5 harness + 架构审查 ✅ → 建议 llman-sdd-verify → archive
```
