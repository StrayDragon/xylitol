...
进行时（打开簇未封口）：Edited→`Editing`，Explored→`Exploring`，Ran→`Running`。禁止再把纯 read 写成 `Editing`。打开簇有工具时画带三角的簇头；流式工具是子项（**默认折叠**，点三角才见正文），不是无三角的 live 尾行。同一打开簇内 ToolEnd **不得**自动收起已展开的子项。Thinking 流画一条 `Thinking`，流结束后更新为 `Thought` / `Thought {dur}`，不得并排 thinking L1 头。**MUST NOT** 画 `Planning next moves`。助手正文始终外显、不进簇。
