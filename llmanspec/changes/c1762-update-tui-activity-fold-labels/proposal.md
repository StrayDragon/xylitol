...
checkpointed: true
checkpoint_sha: 60aa674ae547aeffbea1dfc84d3103a85a2ecf55
---
...
1. **分层词表（Title Case）**，Worked / Explored 不是同一根条的两种文案：
   ...
   - 回退头：仅 thinking → 流式 `Thinking`，结束后 `Thought` / `Thought {dur}`；仅 MCP/未知 → `Used {tool}` 或 `Used N tools`（N=调用次数）；仅 Ask → `Asking questions`。
...
| MCP / 未知 / todo_* | `Used`，N=调用次数（1 次写短名）；checklist 行不计入；永不进 Explored；**不**为 Todo 单开簇头类目 |
...
| 3 | 工具归属 | **已钉**：改写→Edited；read/ls/search→Explored；bash→Ran；MCP/未知/`todo_*`→Used（N=调用次数；不单开 Todo 簇头） |
