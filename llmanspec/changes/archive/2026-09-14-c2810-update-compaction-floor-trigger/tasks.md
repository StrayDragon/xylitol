...
### T1 领域：地板投影 + 有效阈值（含 32k 数值复现）（已完成）
...
### T2 orchestrator：threshold 过闸 + tokens_after / notice 收口（已完成）
...
- **附带收口（用户裁决）：摘要请求输出预算真正接线**——bridge `AiBridgeGenerateOptions` 增 `max_output_tokens: Option<u32>`（None = 现状默认；不过 wire）；三 adapter（anthropic_messages / openai_completions / openai_responses）按 Some 消费；`llm_summarizer::generate_complete` 死参 `_max_tokens` 转正写入 options（0.8× / 0.5× reserve 语义不变）；bridge 侧单测断言请求体字段。
- 撑 spec：`domain-compaction` c2 / c26 / c28。

### T3 runtime：once 旗标接线（已完成）
...
### T4 protocol：CompactionEnd 载荷 + wire 往返（已完成）
...
### T7 lab 回放 harness（#[ignore]，不进 qa）（已完成，落点 tests/inner/ in-crate 挂载）
