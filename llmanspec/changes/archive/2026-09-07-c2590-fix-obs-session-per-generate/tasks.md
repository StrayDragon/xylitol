...
- [x] t2 `AiBridgeGenerateOptions` 携带观测会话快照；adapter/hooks/generation span 使用快照
- [x] t3 单测：两路重叠 generate、不同书签，header 与 `langfuse.session.id` 不错位；进程槽被中途改写不得污染先发出的请求
- [x] t4 `cargo test -p xylitol-ai-bridge` 相关单测 + 既有 otel CollectingReporter + `llman sdd validate c2590-fix-obs-session-per-generate --strict --no-check`
