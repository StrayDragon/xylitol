...
ops[8]{op,req_id,title,statement,from,to,name}:
  add_requirement,m1,"model-registry-providers","ModelRegistry MUST support register_provider(name, config) and has_configured_auth(model) checking for API key or OAuth availability.",null,null,null
  add_requirement,m2,"model-discovery","ModelRegistry MUST support get_available() returning all configured models across registered providers, sorted by provider priority.",null,null,null
  add_requirement,m3,"model-defaults","ModelRegistry MUST maintain default model IDs per provider type (openai: gpt-4o, anthropic: claude-sonnet-4-20250514, etc.).",null,null,null
  add_requirement,m4,"model-resolver","System MUST provide resolve_model(pattern, available) supporting provider/modelId canonical form, bare id matching, alias preference over dated versions, and thinking-level:model pattern parsing.",null,null,null
  add_requirement,m5,"scoped-models","AgentSession MUST support scoped_models (from CLI --models flag) for Ctrl+P model cycling, limiting available models to a user-specified subset with optional per-model thinking levels.",null,null,null
