...
- [x] 2.1 [blocked-by: 1.3] Remote `reload_runtime`：select cancel vs unary；取消分支 drop 原未来后发 `abort` unary（尽力而为）并返回 `cancelled=true`
- [x] 2.2 正常完成路径解析逻辑保持不变（现有行为零回归）

## 3. 护栏与收口

- [x] 3.1 [blocked-by: 2.1] mock HostClient 护栏：reload unary 挂起 → 取消 → 断言 abort 被调用且报告 `cancelled=true`
- [x] 3.2 fmt/lint/test/bdd 全绿；勾 `_TUI_MIGRATED_TODO.md` P1「C3 reload 合作取消」
