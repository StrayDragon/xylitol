...
- [x] `src/app/tui/bridge/preview.rs`：`human_tool_args_preview_with_path` todo 分支
      无 `items` 可解析 → `String::new()`（原 `PATH_PLACEHOLDER`）；有 `items`
      （含空数组 → `0 items`）行为不变
- [x] 单测：`todo_args_preview_is_item_count_not_json` 改断言——`todo_list({})` → ""、
      `todo_update({id,status})` → ""、rewrite 流式缺 `items` → ""、空数组 → `0 items`
- [x] BDD：live spec 新 `@req:att13 @executable` 场景
...
- [x] 验证：`cargo test --lib --all-features tests::bdd::` att13 相关 + preview 单测

## T2 att36：空列表 body 显式空态提示

- [x] `src/app/tui/bridge/preview.rs`：`humanize_tool_result_for_tui` todo 分支空列表
      → `Some("(empty list)")`（固定英文词；is_error 仍原样）；非空清单渲染不变
- [x] 单测：`humanize_todo_result_is_glyph_checklist` 空表断言 `Some("")` →
      `Some("(empty list)")`；补 `output_looks_like_machine_json` 不误判
- [x] BDD：live spec 新 `@req:att36 @executable` 场景
...
- [x] 验证：同上 att36 相关；`bridge/tests.rs` 既有 todo 用例回归
- [blocked-by: T1]
