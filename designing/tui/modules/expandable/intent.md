...
6. 轨色：pending accent / 成功 success / 失败 error。
7. Edit/Diff：header + 正文同一轨；正文 **MUST NOT** 叠 `diff-*-bg` 行底。
8. 摘要已含命令时，展开详情 **MUST NOT** 再 echo 同一命令。bash 走人话 stdout；read 走 content；**MUST NOT** 默认甩 machine JSON。
