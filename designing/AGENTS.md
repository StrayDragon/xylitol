...
Token 色板：[`src/app/tui/DESIGN.md`](../src/app/tui/DESIGN.md) frontmatter（一份）。引擎：`packages/xylitol-tui/AGENTS.md`。产品面：[`src/app/tui/AGENTS.md`](../src/app/tui/AGENTS.md)。设计稿入口即本目录 `tui/modules/<id>/`。
...
- `agent_demo` 不是产品 playground / 固定区真值（硬边界见 `packages/xylitol-tui/AGENTS.md`）。
