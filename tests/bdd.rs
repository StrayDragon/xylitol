...
use xylitol::agent::extensions::{
    Extension, ExtensionContext, ExtensionEvent, ExtensionEventResult, ExtensionLoader,
    ToolDefinition,
};
...
pub struct ExtensionStore {
    pub loader: RefCell<ExtensionLoader>,
    pub ctx: RefCell<Option<ExtensionContext>>,
    pub last_result: RefCell<Option<Result<serde_json::Value, String>>>,
    pub last_block_reason: RefCell<Option<String>>,
    pub last_modified: RefCell<Option<serde_json::Value>>,
}
impl ExtensionStore {
    fn new() -> Self {
        ...
        let ctx = ExtensionContext::new(
            ...
            None,
            Some(Arc::new(tokio_util::sync::CancellationToken::new())),
        );
        Self {
            loader: RefCell::new(ExtensionLoader::new()),
            ctx: RefCell::new(Some(ctx)),
            last_result: RefCell::new(None),
            last_block_reason: RefCell::new(None),
...
struct GreetBddTool;
impl ToolDefinition for GreetBddTool {
    fn name(&self) -> &str {
        "greet"
    }
...
struct BlockBashBddExt;
impl Extension for BlockBashBddExt {
    fn name(&self) -> &str {
        "block-bash"
    }
    fn on_event(
...
struct ModifyOutputBddExt;
impl Extension for ModifyOutputBddExt {
    fn name(&self) -> &str {
        "modify-output"
    }
    fn on_event(
...
#[given("一个扩展注册了自定义工具 {tool_name:string}")]
fn _g_extension_register(ext: &ExtensionStore, tool_name: String) {
    let _ = tool_name;
    struct WrapTool(Arc<dyn ToolDefinition>, String);
    impl Extension for WrapTool {
        fn name(&self) -> &str {
            &self.1
        }
    ...
    ext.loader.borrow_mut().add_extension(Arc::new(WrapTool(
        Arc::new(GreetBddTool),
        "greet-ext".into(),
    )));
}

#[when("调用扩展工具 {tool:string} 参数 {args}")]
fn _w_extension_call_tool(ext: &ExtensionStore, tool: String, args: String) {
    let _ = tool;
    let ctx = ext.ctx.borrow().clone().unwrap();
    let params: serde_json::Value = serde_json::from_str(&strip_quotes(&args)).unwrap();
    let t = GreetBddTool;
    let result = t.execute("call-1", params, None, &ctx);
    ext.last_result.replace(Some(result));
}

#[then("工具返回 {expected}")]
fn _t_extension_tool_result(ext: &ExtensionStore, expected: String) {
    let expected: serde_json::Value = serde_json::from_str(&strip_quotes(&expected)).unwrap();
    let actual = ext.last_result.borrow();
    let actual_ref = actual.as_ref().unwrap().as_ref().unwrap();
    assert_eq!(*actual_ref, expected, "tool result mismatch");
}

#[given("扩展 {name:string} 订阅了 before_tool_call 事件")]
fn _g_extension_block(ext: &ExtensionStore, name: String) {
    let _ = name;
    ext.loader
        .borrow_mut()
        .add_extension(Arc::new(BlockBashBddExt));
}

#[when("尝试调用工具 {tool:string}")]
fn _w_extension_try_call(ext: &ExtensionStore, tool: String) {
    let ctx = ext.ctx.borrow().clone().unwrap();
    let reason =
        ext.loader
            .borrow()
            .check_tool_call_blocked("call-1", &tool, &serde_json::json!({}), &ctx);
    ext.last_block_reason.replace(reason);
}

#[then("工具被阻塞")]
fn _t_extension_blocked(ext: &ExtensionStore) {
    assert!(
        ext.last_block_reason.borrow().is_some(),
        "tool should be blocked"
    );
}

#[then("阻塞原因为 {reason:string}")]
fn _t_extension_block_reason(ext: &ExtensionStore, reason: String) {
    assert_eq!(
        ext.last_block_reason.borrow().clone().unwrap(),
        reason,
        "reason mismatch"
    );
}

#[given("扩展 {name:string} 订阅了 after_tool_call 事件")]
fn _g_extension_modify(ext: &ExtensionStore, name: String) {
    let _ = name;
    ext.loader
        .borrow_mut()
        .add_extension(Arc::new(ModifyOutputBddExt));
}

#[when("工具 {tool:string} 返回 {result}")]
fn _w_extension_tool_returns(ext: &ExtensionStore, tool: String, result: String) {
    let _ = tool;
    let ctx = ext.ctx.borrow().clone().unwrap();
    let input: serde_json::Value = serde_json::from_str(&strip_quotes(&result)).unwrap();
    let modified = ext
        .loader
        .borrow()
        .modify_tool_result("call-1", "read", &input, false, &ctx);
    if let Some(m) = modified {
        ext.last_modified.replace(m.content);
    }
}

#[then("结果被修改为 {expected}")]
fn _t_extension_result_modified(ext: &ExtensionStore, expected: String) {
    let expected: serde_json::Value = serde_json::from_str(&strip_quotes(&expected)).unwrap();
    assert_eq!(
        ext.last_modified.borrow().clone().unwrap(),
        expected,
...
#[given("扩展上下文包含中断信号")]
fn _g_extension_signal(ext: &ExtensionStore) {
    let token = tokio_util::sync::CancellationToken::new();
    ext.ctx.borrow_mut().as_mut().unwrap().signal = Some(Arc::new(token));
}

#[when("调用 abort")]
fn _w_extension_abort(ext: &ExtensionStore) {
    ext.ctx.borrow().as_ref().unwrap().abort();
}

#[then("is_aborted 返回 true")]
fn _t_extension_aborted(ext: &ExtensionStore) {
    assert!(
        ext.ctx.borrow().as_ref().unwrap().is_aborted(),
        "expected is_aborted=true"
    );
}
...
// extensions.feature (4)
#[scenario(path = "tests/features/extensions.feature", name = "注册自定义工具")]
fn test_extension_register_tool(ext: ExtensionStore) {
    let _ = ext;
}
#[scenario(path = "tests/features/extensions.feature", name = "阻塞工具调用")]
fn test_extension_block_tool(ext: ExtensionStore) {
    let _ = ext;
}
#[scenario(path = "tests/features/extensions.feature", name = "修改工具结果")]
fn test_extension_modify_result(ext: ExtensionStore) {
    let _ = ext;
}
#[scenario(
    path = "tests/features/extensions.feature",
    name = "扩展上下文中断信号"
)]
fn test_extension_abort_signal(ext: ExtensionStore) {
    let _ = ext;
}
