...
- `just qa` 在 workspace 测试之后串行跑 `test-live-provider`（`--test-threads=1`，不进 nextest 并行矩阵）；专用配置 `<global-dir>/dev/live-provider.yaml`（全局目录：`$XYLITOL_CONFIG_DIR` → `$XDG_CONFIG_HOME/xylitol` → `~/.config/xylitol`，经 dotxylitol 多机共享；示例由 `just gen-live-provider-example` 自动生成；`enabled: true` 才实打网关；缺失/关闭则 skip）。
