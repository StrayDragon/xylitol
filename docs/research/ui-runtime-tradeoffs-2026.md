...
2. **Web TS 面出局**：c2310（薄 TS 客户端）draft 删除；Cloud-Agent 控制台 roadmap 搁置；`bindings.ts` + `gen-sdk` + pa-bind1 drift 闸保留为协议类型保险（≈0 成本、不依赖消费者存在）。
...
| 类型 | Rust 直 `use` 协议闭集 | protocol 编译 wasm target 可复用，但 UI 框架生态 churn 大 | bindings.ts + 薄客户端 |
...
- TS 类型保险：`packages/xylitol-client-typescript-sdk/`（`just gen-sdk`，pa-bind1）
