...
| 侧 | 关注点 |
|---|---|
| **docs（产品）** | 用户碰到什么、领域词汇、开箱/后置、跨端语义 |
| **llmanspec（实现）** | 可验证行为合约、变更原子性、BDD 场景真源 |

### 跨端公共体验（写文档时）

TUI 与第二产品端（gpui 桌面）等**共有**能力：产品文必须按「一套学习成本」叙述，语义同源、端专属才分叉——MUST / 禁止细则**唯一**约束板：[`roadmaps/跨端同源.md`](./roadmaps/跨端同源.md)（本节不复述规则，只定 docs 落点）。

| 写哪 | 怎么写 |
|---|---|
| 未兑现公共交互 | `roadmaps/跨端同源.md`（及衔接篇）；可链 `llmanspec` 草案 |
| 已兑现公共心智 | `architecture/`（如多客户端）；勿把未开放端写成现行 MUST |
| 端专属（纯 TTY / 纯 GUI） | 可分叉；只在该端自己的文档讲 |

改公共交互相关 roadmap / architecture / 各端 `AGENTS` 前，先按约束板「产品规则」对齐全部端。
...
- 根 `AGENTS.md`：全仓工作原则；产品图入口指向本目录；含跨端公共体验一行指针。
- `src/app/tui/AGENTS.md`：TUI 端边界 + 跨端公共体验操作约束；固定区词表指针 → `architecture/TUI信息呈现与固定区词汇.md`。
...
- change 临时调研 / 选型深挖：落点规则见 `llmanspec/AGENTS.md`（勿堆 `docs/research/`）。
