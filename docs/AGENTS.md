...
TUI 与第二产品面（gpui 桌面）等**共有**能力：产品文必须按「一套学习成本」叙述，语义同源、面专属才分叉——MUST / 禁止细则**唯一**约束板：[`roadmaps/跨面同源.md`](./roadmaps/跨面同源.md)（本节不复述规则，只定 docs 落点）。

| 写哪 | 怎么写 |
|---|---|
| 未兑现公共交互 | `roadmaps/跨面同源.md`（及衔接篇）；可链 `llmanspec` 草案 |
| 已兑现公共心智 | `architecture/`（如多客户端）；勿把未开闸面写成现行 MUST |
| 面专属（纯 TTY / 纯 GUI） | 可分叉；只在该面自己的文档讲 |

改公共交互相关 roadmap / architecture / 面 `AGENTS` 前，先按约束板「产品规则」对齐全套面。
