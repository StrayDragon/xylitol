...
> 在已落地「reserve 预算 auto-compact + 诚实 provenance」之上，把 **provider prompt cache、动态压缩策略、工具/会话噪声压缩** 做成可感知、可度量的省 token / 降延迟能力。
> 现状对齐：2026-09-13。已兑现心智见 [../architecture/压缩与上下文.md](../architecture/压缩与上下文.md)；本文只留未兑现候选。
...
|---|---|
| reserve 预算触发自动压缩（占用 > 窗口 − reserve，可配置） | **动态**阈值 / 按段策略 / 可声明保留锚点 |
...
| 与 Eval 回归集可验证「省 token 不损任务」 | 为省钱牺牲默认正确性且无门禁 |
...
| 阶段 | 用户可感知结果 |
|---|---|
| **M1 Cache 透出与诚实用量** | 兼容族透传/映射 cache 相关 usage（读缓存 / 不适用 / 未上报 三态诚实映射）**已落地**（c1885 responses-cache-usage-honesty，见 [`../architecture/压缩与上下文.md`](../architecture/压缩与上下文.md)）；候选只剩：footer / 观测可区分「读到缓存」vs「未知」的透出 |
...
| [Agent-Eval与回归基准.md](./Agent-Eval与回归基准.md) | 策略变更的可验证门禁 |
...
| **多模态体积门禁** | 音视频附件与 cache/compact 的配额协同（挂多模态） |
