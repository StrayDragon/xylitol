...
| agent 忙碌时切模型 / 看主题 / 浏览会话列表 | 即时列表可开 | Resume **可浏览**、**不可 switch**（壳层通告说明） |
| busy 时误点切换会话 | 壳层通告拒闸 | 须等 turn 结束或 Esc 中止后再 switch |
| 改键 | 只能手改 JSON | 后期可配置 UI / Web；短期 JSON 仍够用 |
| 模拟按键学习 | 无交互键盘图 | **延后**：Web 面；见调研 |

现行拒闸文案与落点：见 [../architecture/TUI信息面与chrome词汇.md](../architecture/TUI信息面与chrome词汇.md)（壳层通告）与 [`designing/tui/modules/chrome-toast/`](../../designing/tui/modules/chrome-toast/intent.md)。
...
- **学键主路径**：文档 / 日后 Web 学习页；组件键写在 [`designing/tui/modules/`](../../designing/tui/modules/) 各模块 `draft.yaml` 的 `keys:`。**不**把程序内百科当默认交付。
...
1. M4 是否允许「浏览器打开本地静态页」作为 Web 壳前过渡？
