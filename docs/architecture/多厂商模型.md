...
flowchart LR
  ...
  Adapters --> A["OpenAI 兼容 · Completions / Responses"]
...
3. **Pre-1.0 交付只两家**：OpenAI 兼容（**Chat Completions 与 Responses**）与 Anthropic（Messages）。用户自定义端点仅当声明为上述兼容 API 时接受。
...
| | 内容 |
|---|---|
| **主线** | OpenAI 兼容（Completions / Responses）+ Anthropic；统一事件与工具调用体验；thinking 经 `/model`（默认支持集最高档） |
