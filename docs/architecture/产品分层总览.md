...
flowchart TB
  ...
  subgraph Later["有需要再打开"]
    ...
    MCP["按配置启用的外部工具（MCP）"]
    Remote["远程协作体验打平"]
    Next["/reload · skills UX · 粘贴等（drafts）"]
  end
...
| **后置 / 配置启用** | Server · MCP · 更多厂商 · 周边；未配置则不装配 |
| **已开闸主路径** | 产品 TUI（Driver + 事件流；会话树 / slash / bang 等已可用） |
| **下一波缺口** | `/reload`、skills 视觉、`$skill`、thinking 边框、粘贴图片等 → purpose-drafts `c1080`–`c1160` |
