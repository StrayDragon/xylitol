...
> 现状对齐：2026-07-24（仅 config.yaml + secret.env）。
...
- **API 类型**：OpenAI 兼容默认为 **`openai-responses`**；显式可识别另含 **`openai-completions`**；Anthropic 为 `anthropic-messages`。配置真值用全称。命名 `models.*.compat`（`generic` / `deepseek`）选 wire/thinking 轮廓；自由形式 `extra_policy` **不**进 YAML。per-model `api_key`（可 `{{ secret.* }}`）优先于 kind 级 env。
...
| Pre-1.0 只接受 OpenAI 兼容与 Anthropic；OpenAI 兼容**默认 Responses**，Completions 须显式 `api`；命名 `compat` 可选 | 为未交付厂商堆配置产品概念；把 Completions 当开箱默认；静默吞 Completions |
