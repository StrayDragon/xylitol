...
- **API 类型**：OpenAI 兼容默认 **`openai-responses`**；Completions 为遗留显式档（`api: openai-completions`）；Anthropic 为 `anthropic-messages`。配置真值用全称。未暴露的 wire 保守策略（`compat` / `extra_policy`）在 bridge `defaults.rs`，**不**进本配置面 YAML/env。
...
| Pre-1.0 只接受 OpenAI 兼容与 Anthropic；OpenAI 兼容**默认 Responses**，Completions 须显式声明 | 为未交付厂商堆配置产品概念；把 Completions 当开箱默认 |
