...
- **API 类型**：OpenAI 兼容默认为 **`openai-responses`**；显式可识别另含 **`openai-completions`**；Anthropic 为 `anthropic-messages`。配置真值用全称。命名 `models.*.compat`（`generic` / `deepseek`）选 wire/thinking 轮廓；自由形式 `extra_policy` **不**进 YAML。per-model `api_key`（可 `{{ secret.* }}`）优先于 kind 级 env。`thinking_levels` 为该模型用户面档位 SSOT（未声明 → 仅 `off`）；例 DeepSeek `[off, low, high, max]`（见 `configs/example.yaml`）。
