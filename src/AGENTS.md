...
本文件是 `src/` **代码架构**的单一真值源：依赖方向、seam、`Xy*`、消息/工具边界、扩展开闭、复杂度与体量策略。全局工作方式见根 `AGENTS.md`；产品心智见 `docs/architecture/`。子目录 `AGENTS.md` 只写本端例外，不重复本文件。
...
app → agent → protocol/{wire, ports, model, session, …}
  ...
  └──── infra ────┘
         ↘
        utils   (叶：仅 std；各层可依赖；非 `Xy*` 稳定 API)
```
...
| `app` | 应用端 + `core` 跨端 seam | 走 seam，不 reach `agent`/`infra` 内部；**产品 slash / bang / session export** 在 `app/core` + `XyDriver`（端经 Driver，不进 capabilities） |
...
| 规则（MUST） | 反例（MUST NOT） |
|---|---|
| 一个 runtime **显式绑定一个 session** 后才可根提交；任意时刻至多一个 ReAct worker 可写该 session（单写者，single-writer） | 在同一 runtime 按 session id 多路复用；共享 history / cancel / active turn / 插话队列 |
| 根提交默认拒绝忙碌并发；busy 时 `steer` / `follow_up` 是轮内改向，不是第二次根提交 | 忙碌中塞第二次根提交；把 steer / follow_up 当新根提交 |
| 未来子 agent = **另建隔离 runtime** | 子 agent 挤进现有 runtime 共享状态 |
| Host 进程可 lazy 物化 N 个 session 槽 Driver，共享同一份 `RuntimePorts` 基线 | 把「多 Driver 共享基线」当成「一把 Driver 多路复用两个 session」；单把 Driver 同时两次根提交 / 同时绑两个 session |
| `RunId` 仅运行时内部 | `RunId` 进 `XyEvent` / wire |
...
- **schemars**：放 infra / config 侧；protocol 根类型默认只 serde。
...
- 应用级接缝（seam）：`XyDriverError`（含 `Agent(…)`）+ `kind` / `detail_kind` / `log_failure`。session/export/trust 失败 flatten 后 `kind` 仍是 Driver 分类（`NotFound`/`Io`/…），`detail_kind` 与 `log_failure` 的 `source.kind` 保留来源域（`Session`/`Export`/`Trust`）。opaque 字符串经 `from_opaque`（具体短语，避免裸 token 误伤）。
...
禁止：插件市场、Extension Host、为未交付能力预留空壳 `Xy*`、「已是统一口再包一层」；改名不留兼容别名 / 双解析路径（一次性改调用点）。死码分诊走 skill `l8ng-audit-dead-code`。

Trust / Permission / MCP 产品语义分别见 `docs/architecture/` 对应文；实现要点：Trust 门禁**项目本地资源**；工具开箱 allow-all；MCP **有配置才装配**并支持重载。
...
protocol **MAY** 依赖 bridge **DTO only**，**MUST NOT** 依赖 bridge HTTP/SDK。新兼容端 = adapter/配置；**不改** ReAct / `AgentMessage`。当前交付范围见 `docs/architecture/多厂商模型.md`。
...
**TUI 端复杂度门禁（HARD）**：产品 TUI 入口协调者（host 入口、layout 根、effects 入口、bridge 入口）的函数级复杂度经 `just qa` 的 `scripts/check_complexity.py`（cccc-rs）强制：Sonar cognitive ≤32 且 McCabe cyclomatic ≤27。**不以文件物理行数作为硬门禁**（行数硬测会逼出按 impl 块散落的假拆分）。测试专用模块不参与本门禁。

**全树复杂度雷达（review 信号）**：`just complexity`（`--radar`）对全生产树（排除测试文件）报告 cccc-rs top-cognitive 排名，作为 review 参照；软信号，非硬门禁。复杂度高的函数优先拆分/重构，不用行数 KPI。

**默认不为行数大拆**：ReAct（剧本可与行为测同居；state machine 未开放）、session manager、已拆开的 driver 子树。功能逼出或编辑痛点明确时再拆；复杂度超门禁须拆分或显式豁免。**不**在本文件维护超标清单。
...
- 主门禁 `just qa` 走 nextest（一测一进程）：勿假设 in-process `#[serial]` 跨测生效。无固定端口 / 共享非 temp 路径等外部共享资源时**不**配置 `[test-groups]`；出现再开。
