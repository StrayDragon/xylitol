...
**产品 TUI（`src/app/tui`）**：**已开闸（2026-07-11）**。轨 B MVP 至 **c485** 已归档；下一：`c492-add-app-tui-bash-mode` / `c493-add-app-tui-compaction-retry-ui`（purpose-draft）。引擎能力仍可在 `packages/xylitol-tui` / `agent_demo` 先行验证。c491 假树 stub 禁止扩活树。
