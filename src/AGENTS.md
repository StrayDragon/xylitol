...
- 产品 `HostSession` 经 `KeybindingsScope`（thread-local）持有键位；勿在 `new_product_ui` 路径无必要写 `GLOBAL_KEYBINDINGS`。配置发现优先 `ConfigPaths::discover_with` / `load_app_config_*_with` 注入，少用 `set_var`；`secret.env` 注入进程环境的测仍走 `env_global`。测试勿把 `SessionManager` 指到开发者真实 `~/.xylitol/sessions`（用 tempfile）。
