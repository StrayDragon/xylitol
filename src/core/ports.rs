...
// ── Sandbox ───────────────────────────────────────────────────────────
//
// The sandbox port + verdict vocabulary. Concrete backends (FallbackBackend,
// NoopEngine, platform Landlock/macOS) live in `infra::sandbox`; both `agent`
// and `infra` reference the trait/verdict from here so agent need not reach
// into infra for the type.
