...
| 空 `!` / `!!` | 系统提示，不执行 |
| 结果 | live scrollback：`UiEntry::Bash` 块（`$ cmd` + 输出）；提交即 **pending** 轨色；输出 chunk 到达时 **同一块内增量刷新**（保持 pending 轨）；成功 → success 轨；失败 / Esc `(cancelled)` → error 轨；经包 `paint_left_rail_line`（轨+gutter+内容）；**MUST NOT** 默认整行 `tool-*-bg` 洗底 |
...
bash 交互块用 **status 轨三态**（pending / success / error·cancelled）；边框仍用 `{colors.success}` fg 强调 `!` 模式。退出码与 `(cancelled)` 用 error 轨 + error fg。
