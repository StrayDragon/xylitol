...
5. **展开后形态**（自上而下）：
   1. 统计摘要头：文件层互斥 `Editing {name|N files}` 或 `Exploring {name|N files}`，有 shell 才追加 `Running N commands`，可选 `{colors.success}` `+N` / `{colors.error}` `-N`（无可靠 diff 则省略 +/-）。MUST NOT 并列 explored，MUST NOT 把纯 read 写成 Editing。thinking 与工具同簇：簇头走真实活动（`Used` 等），**MUST NOT** 用 `Thought` 当聚合头。仅 thinking：流式为 `Thinking`，结束后为 `Thought` / `Thought {Ns}`。
