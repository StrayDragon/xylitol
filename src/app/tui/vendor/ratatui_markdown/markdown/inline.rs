...
use crate::app::tui::vendor::ratatui_markdown::theme::RichTextTheme;
...
pub fn parse_inline_formatting(text: &str, theme: &impl RichTextTheme) -> Vec<Span<'static>> {
    ...
    while i < len {
        ...
        // <url> autolink: render as bare URL (no angle brackets).
        if chars[i] == '<' {
            let mut end = i + 1;
            ...
            if end < len && chars[end] == '>' {
                ...
                if inner.contains("://") || inner.starts_with("mailto:") {
                    ...
                    spans.push(Span::styled(inner, url_style));
                    i = end + 1;
                    continue;
                }
            }
            // Not a valid autolink — fall through to treat '<' as literal.
        }
            ...
            while end_bracket < len {
                if chars[end_bracket] == '[' {
                    ...
                    if bracket_depth > 0 {
                        ...
                        flush_current!();
                        // codex-style "text (url)" so the URL is visible and
                        // manually copyable in terminals without OSC8 support.
                        let url_style = Style::default()
                            .fg(theme.get_primary_color())
                            .add_modifier(Modifier::UNDERLINED);
                        let paren_style = Style::default().fg(theme.get_muted_text_color());
