...
- **鼠标**：产品默认 **ApplicationOwned**（alt-screen）：会话 begin 后开 mouse capture，应用内选区 + dock 排除输入面；**不**读 `XYLITOL_TUI_MOUSE`（该 env 仅库 lab / e2e / Inline demo）。库仍暴露 Inline 构造入口。模式在 host **启动构造**时绑定（`new_product_ui_with_meta_mode` / `TuiRunOptions`，缺省 ApplicationOwned）；**禁止** mid-session 热切 / 再引入 `apply_interaction_mode`。折叠三角列点击（Tool/Diff/Ask/Thinking per-id）经 `set_transcript_hit_priority` 接线（[`c2040`](../../../llmanspec/changes/c2040-add-tui-mouse-click-fold-triangle/proposal.md)；库双模式见 archived [`c2070`](../../../llmanspec/changes/archive/2026-08-12-c2070-add-package-tui-dual-interaction-modes/proposal.md)）。剩余块 / 更广 `FoldTarget` 见 [`c2045`](../../../llmanspec/changes/c2045-add-tui-fold-target-remaining/proposal.md)。命名：用 `Inline` / `ApplicationOwned`，勿写 `mode_a`/`mode_b`（见 `packages/xylitol-tui/AGENTS.md` §硬约束 8）。
