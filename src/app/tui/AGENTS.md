...
- **鼠标**：产品 inline 面 **不**开 mouse capture，也 **不**读 `XYLITOL_TUI_MOUSE`。该 env 仅 **lab / e2e**（`agent_demo`、PTY）；包仍保留 `enable_mouse_capture` API。正式鼠标 UX（应用内选区 / 点折叠）延后至双模式 Mode B——[`c2070`](../../../llmanspec/delayed-changes/tui/c2070-add-package-tui-dual-interaction-modes/proposal.md)。`InputEvent::Mouse` 扇入可保留（无 capture 时通常无事件）；禁止在产品路径为「试鼠标」默认 Enable。
