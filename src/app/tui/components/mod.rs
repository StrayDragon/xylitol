...
//! (`RenderedLine` / `&str` / spinner index / input buffer) and renders into a
//! `ratatui_core::buffer::Buffer` via the `Widget` trait — so it is verifiable
//! with a `TestBackend` and composable by the layout widget [`tail::Tail`].
//! No widget imports agent/infra internals or matches `XyEvent` (spec
//! tui41/tui42). The business→UI seam (`xyevent_to_rendered`) stays in
