...
<tool_call>{"name": "b", "arguments": {"x": 1}}</tool_call>"#;
    ...
    #[test]
    fn test_buffer_preserves_newline_tokens() {
        let mut buf = ToolCallBuffer::new();

        // Simulate streaming: "# Title" + "\n" + "content"
        match buf.push("# Title") {
            BufferAction::Emit(parts) => {
                assert_eq!(parts.len(), 1);
                assert!(matches!(&parts[0], Part::Text { text } if text == "# Title"));
            }
            BufferAction::Buffering => panic!("should emit"),
        }

        // Newline as separate token — must NOT be dropped
        match buf.push("\n") {
            BufferAction::Emit(parts) => {
                assert_eq!(parts.len(), 1);
                assert!(matches!(&parts[0], Part::Text { text } if text == "\n"));
            }
            BufferAction::Buffering => panic!("newline should emit, not be dropped"),
        }

        match buf.push("content") {
            BufferAction::Emit(parts) => {
                assert_eq!(parts.len(), 1);
                assert!(matches!(&parts[0], Part::Text { text } if text == "content"));
            }
            BufferAction::Buffering => panic!("should emit"),
        }
    }
    ...
    #[test]
    fn test_buffer_preserves_double_newline() {
        let mut buf = ToolCallBuffer::new();
        match buf.push("\n\n") {
            BufferAction::Emit(parts) => {
                assert_eq!(parts.len(), 1);
            ...
            BufferAction::Buffering => panic!("double newline should emit"),
        }
    }
}
